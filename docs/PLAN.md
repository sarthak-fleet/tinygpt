...
- ⬜ **E9. Prompt-tiering A/B on the planner unhappy suite** — hypothesis from the v9-compose regression: always-resident action-registry detail biases non-action intents (the "L1 bloat" failure mode — see `docs/learn/agent-context-hierarchy.md`). Build a v11 variant whose action registry is a compact name-only index, run the n=130 drill vs the full prompt. If ambig/OOS move ≥5pp, tiered prompts become the default for every future planner prompt. ~half-day with the existing `eval_planner.sh` harness; failure-pattern triage (shipped 2026-06-12) tells you *which* intents the registry distorts.
...
- ⬜ **B26. Server-side deferred tools in `tinygpt serve`** (added 2026-06-12) — `--tools` today injects the full catalog into every request's system prompt (`ServeToolsSpec.systemPrompt()`, `Serve.swift`). Steal from the Shortcut vertical-agents essay (see `docs/learn/agent-context-hierarchy.md`): inject only a one-line-per-tool index plus a built-in `get_tool_info(name)` meta-tool that serve answers itself (returns the full schema as the tool result, session-cached). Large catalogs then cost ~0 prompt tokens until used, and any OpenAI-compatible client gets it for free since the round-trip is an ordinary tool call. Needs a numerics-style gate: BFCL score with deferred catalog must match full-catalog within noise before it can default on. ~2 days.
